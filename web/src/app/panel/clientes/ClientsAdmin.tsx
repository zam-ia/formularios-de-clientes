...
export default function ClientsAdmin({ readOnly = false }: { readOnly?: boolean }) {
  ...
  return <main className={styles.osPage}>
    ...
    {loading ? <div className={styles.loadingBlock}><LoaderCircle className={styles.spin} /> Cargando clientes…</div> : visible.length ? <section className={styles.clientGrid}>{visible.map((client) => <article className={styles.clientCard} key={client.id}><header><span className={styles.clientAvatar}>{client.company_name.slice(0, 2).toUpperCase()}</span><div><strong>{client.company_name}</strong><small>{client.contact_name}</small></div><em className={styles[client.status]}>{statusLabels[client.status]}</em></header><div className={styles.clientPlan}><span>PLAN ACTUAL</span><strong>{client.plan_name}</strong><small>{money(client.monthly_fee, client.currency)} · {accountLabels[client.payment_account]}</small></div><div className={styles.clientDates}><span><small>Inicio</small>{client.start_date}</span><span><small>Finaliza</small>{client.end_date}</span></div><footer>{client.whatsapp ? <a href={`https://wa.me/${client.whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noreferrer"><MessageCircle /> WhatsApp</a> : <span /> }<Link href="/panel/finanzas"><CircleDollarSign /> Finanzas</Link>{readOnly ? null : <button onClick={() => setForm({ ...client })}><Pencil /> Editar</button>}</footer></article>)}</section> : <div className={styles.emptyState}><UsersRound /><h2>Aún no hay clientes.</h2><p>Registra el primero para seleccionarlo al crear actividades y movimientos.</p>{readOnly ? null : <button className={styles.primaryAction} onClick={() => setForm(blankClient())}><Plus /> Registrar cliente</button>}</div>}
