...
type DataResponse = { quotes: QuoteView[]; plans: QuotePlan[]; discounts: DiscountRule[]; services: CatalogService[]; categories: ServiceCategory[]; clients: AgencyClient[]; advisors: AdvisorOption[]; settings: CommercialSettings; currentRole: AdminRole; currentUserId: string; currentUserName: string; canAssign: boolean };
...
export default function QuoteAdmin() {
  ...
  const load = useCallback(async () => { const data = await api<DataResponse>("/api/admin/quotes"); setQuotes(data.quotes); setPlans(data.plans); setDiscounts(data.discounts); setServices(data.services); setCategories(data.categories); setClients(data.clients); setAdvisors(data.advisors); setSettings(data.settings); setCurrentRole(data.currentRole); setCurrentUserId(data.currentUserId); setCurrentUserName(data.currentUserName); setCanAssign(data.canAssign); }, []);
  useEffect(() => {
    let cancelled = false;
  ...
  function editQuote(quote: QuoteView) {
    const { id, client_id, client_name, company_name, client_whatsapp, client_email, title, introduction, currency, items, strategies, global_discount_type, global_discount_value, tax_percent, valid_until, terms, notes, status, advisor_id, advisor_name } = quote;
    setEditor({ id, client_id, client_name, company_name, client_whatsapp, client_email, title, introduction, currency, items, strategies, global_discount_type, global_discount_value, tax_percent: tax_percent ?? settings.default_tax_percent, valid_until, terms, notes, status, advisor_id: advisor_id || quote.created_by, advisor_name: advisor_name || quote.created_by_name });
  }
  function startQuote() { setEditor({ ...blankQuote(), tax_percent: settings.default_tax_percent, advisor_id: currentUserId, advisor_name: currentUserName }); }
  ...
  function selectClient(clientId: string) {
    ...
    if (!editor) return;
    setEditor(client ? { ...editor, client_id: client.id, company_name: client.company_name, client_name: client.contact_name, client_whatsapp: client.whatsapp, client_email: client.email, advisor_id: client.advisor_id || editor.advisor_id, advisor_name: client.advisor_name || editor.advisor_name } : { ...editor, client_id: undefined });
  }
  ...
  async function saveQuote() {
    ...
    try {
      ...
      const payload = { ...editor, global_discount_value: Math.round(editor.global_discount_value), tax_percent: Math.round(editor.tax_percent || 0), items: editor.items.map((item) => ({ ...item, quantity: Math.max(1, Math.round(item.quantity)), unit_price: Math.round(item.unit_price), discount_percent: Math.round(item.discount_percent), base_cost: typeof item.base_cost === "number" ? Math.round(item.base_cost) : undefined, tax_percent: typeof item.tax_percent === "number" ? Math.round(item.tax_percent) : undefined })) };
      const result = await api<{ quote: QuoteView }>("/api/admin/quotes", { method, body: JSON.stringify({ kind: "quote", data: payload }) });
      setQuotes((current) => [result.quote, ...current.filter((quote) => quote.id !== result.quote.id)]);
      setEditor(null); setNotice(`${result.quote.quote_number} guardada. La lista ya está actualizada y el enlace está listo.`);
  ...
  async function savePlan() {
    ...
    setBusy(true); setNotice("");
    try { const result = await api<{ plan: QuotePlan }>("/api/admin/quotes", { method: plans.some((plan) => plan.id === planEditor.id) ? "PUT" : "POST", body: JSON.stringify({ kind: "plan", data: { ...planEditor, price: Math.round(Number(planEditor.price)) } }) }); setPlans((current) => [...current.filter((plan) => plan.id !== result.plan.id), result.plan].toSorted((a, b) => a.price - b.price)); setPlanEditor(null); setNotice("Plan guardado y visible de inmediato."); void load().catch(() => undefined); }
  ...
  async function saveDiscount() {
    ...
    setBusy(true); setNotice("");
    try {
      const result = await api<{ discount: DiscountRule }>("/api/admin/quotes", { method: discounts.some((discount) => discount.id === discountEditor.id) ? "PUT" : "POST", body: JSON.stringify({ kind: "discount", data: { ...discountEditor, value: Math.round(Number(discountEditor.value)) } }) });
      setDiscounts((current) => [result.discount, ...current.filter((discount) => discount.id !== result.discount.id)]); setDiscountEditor(null); setNotice("Descuento guardado y listo para aplicar."); void load().catch(() => undefined);
  ...
  return <main className={styles.osPage}>
    ...
    {loading ? <div className={styles.loadingBlock}><LoaderCircle className={styles.spin} /> Preparando cotizador…</div> : tab === "quotes" ? <section className={styles.quoteList}>
      {quotes.length === 0 && <div className={styles.emptyState}><Sparkles /><h2>Tu primera propuesta empieza aquí.</h2><p>Crea una cotización, agrega planes o servicios y comparte un enlace elegante con el cliente.</p><button className={styles.primaryAction} onClick={startQuote}>Crear cotización</button></div>}
      {quotes.map((quote) => { const quoteTotal = quoteTotals(quote); return <article className={styles.quoteRow} key={quote.id}><div className={styles.quoteNumber}>{quote.quote_number}</div><div><strong>{quote.company_name}</strong><span>{quote.client_name} · Asesor: {quote.advisor_name || quote.created_by_name} · Válida hasta {quote.valid_until}</span></div><div className={styles.quoteValue}><strong>{money(quoteTotal.total, quote.currency)}</strong><span className={`${styles.statusPill} ${styles[quote.status]}`}>{statusLabels[quote.status]}</span></div><div className={styles.rowActions}><button onClick={() => editQuote(quote)} title="Editar"><Pencil /></button><button onClick={() => copyLink(quote.public_url)} title="Copiar enlace"><Copy /></button><a href={quote.public_url} target="_blank" rel="noreferrer" title="Abrir cotización"><ExternalLink /></a><button onClick={() => void removeQuote(quote.id)} title="Eliminar"><Trash2 /></button></div></article>; })}
    ...
    {editor && <div className={styles.editorOverlay}><section className={styles.editorPanel}>
      ...
      <div className={styles.editorBody}>
        <section className={styles.formSection}><div className={styles.sectionTitle}><span>01</span><div><h3>Cliente y enfoque</h3><p>Selecciona un cliente registrado o completa los datos manualmente.</p></div></div><div className={styles.formGrid}><label className={styles.fullField}>Cliente del CRM<select value={editor.client_id || ""} onChange={(event) => selectClient(event.target.value)}><option value="">Cotización para cliente nuevo</option>{clients.map((client) => <option value={client.id} key={client.id}>{client.company_name} · {client.contact_name}</option>)}</select></label>{canAssign ? <label className={styles.fullField}>Asesor responsable<select value={editor.advisor_id || ""} onChange={(event) => { const advisor = advisors.find((item) => item.id === event.target.value); setEditor({ ...editor, advisor_id: event.target.value, advisor_name: advisor?.displayName || "" }); }}>{advisors.map((advisor) => <option key={advisor.id} value={advisor.id}>{advisor.displayName}</option>)}</select><small>La venta y su comisión se atribuirán a este asesor.</small></label> : <label className={styles.fullField}>Asesor responsable<input value={editor.advisor_name || currentUserName} disabled /></label>}<label>Empresa<input value={editor.company_name} onChange={(event) => setEditor({ ...editor, company_name: event.target.value })} /></label><label>Persona de contacto<input value={editor.client_name} onChange={(event) => setEditor({ ...editor, client_name: event.target.value })} /></label><label>WhatsApp<input value={editor.client_whatsapp} onChange={(event) => setEditor({ ...editor, client_whatsapp: event.target.value })} /></label><label>Correo<input type="email" value={editor.client_email} onChange={(event) => setEditor({ ...editor, client_email: event.target.value })} /></label><label className={styles.fullField}>Título<input value={editor.title} onChange={(event) => setEditor({ ...editor, title: event.target.value })} /></label><label className={styles.fullField}>Introducción<textarea value={editor.introduction} onChange={(event) => setEditor({ ...editor, introduction: event.target.value })} /></label></div></section>
        <section className={styles.formSection}><div className={styles.sectionTitle}><span>02</span><div><h3>Servicios e inversión</h3><p>Elige primero un plan o agrega servicios puntuales.</p></div></div><div className={styles.quotePlanGrid}>{plans.filter((plan) => plan.active).map((plan) => <button key={plan.id} onClick={() => addPlanToQuote(plan)}><span>{plan.badge || "PLAN"}</span><strong>{plan.name}</strong><small>{plan.description || `${plan.features.length} beneficios incluidos`}</small><em>{money(plan.price, editor.currency)}</em><Plus /></button>)}<button onClick={() => setEditor({ ...editor, items: [...editor.items, blankItem()] })}><span>PERSONALIZADO</span><strong>Servicio libre</strong><small>Agrega un concepto que todavía no está en el catálogo.</small><em>Definir precio</em><Plus /></button></div><div className={styles.quoteCatalogFilters}><div><Search /><input value={serviceQuery} onChange={(event) => setServiceQuery(event.target.value)} placeholder="Buscar un servicio adicional…" /></div><select value={serviceCategory} onChange={(event) => setServiceCategory(event.target.value)}><option value="all">Todas las categorías</option>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div className={styles.servicePicker}>{visibleServices.map((service) => <button key={service.id} onClick={() => addServiceToQuote(service)}><span><b>{service.code}</b><small>{categories.find((item) => item.id === service.category_id)?.name}</small></span><strong>{service.name}</strong><em>{money(service.suggested_price, editor.currency)} · {service.unit}</em><Plus /></button>)}</div><div className={styles.itemStack}>{editor.items.map((item, index) => { const service = services.find((current) => current.id === item.service_id); const maxDiscount = currentRole === "sales" && service ? service.max_discount_percent : 100; return <article className={styles.itemEditor} key={item.id}><div className={styles.itemHead}><strong>{item.code ? `${item.code} · ` : ""}Servicio {index + 1}</strong><button onClick={() => setEditor({ ...editor, items: editor.items.filter((current) => current.id !== item.id) })} disabled={editor.items.length === 1}><Trash2 /></button></div><div className={styles.formGrid}><label className={styles.fullField}>Nombre<input value={item.name} onChange={(event) => updateItem(item.id, { name: event.target.value })} /></label><label className={styles.fullField}>Descripción<textarea value={item.description} onChange={(event) => updateItem(item.id, { description: event.target.value })} /></label><label>Cantidad<input type="number" min="1" step="1" value={item.quantity || ""} placeholder="1" onChange={(event) => updateItem(item.id, { quantity: Number(event.target.value) })} /></label><label>Precio unitario<input type="number" min="0" step="1" value={item.unit_price || ""} placeholder="Ej. 500" onChange={(event) => updateItem(item.id, { unit_price: Number(event.target.value) })} /></label><label>Descuento del servicio (%)<input type="number" min="0" max={maxDiscount} step="1" value={item.discount_percent || ""} placeholder="0" onChange={(event) => updateItem(item.id, { discount_percent: Number(event.target.value) })} /><small>Límite para tu rol: {maxDiscount}%</small></label>{canManageCommercial && typeof item.base_cost === "number" ? <label>Margen estimado<strong className={item.unit_price * (1 - item.discount_percent / 100) >= item.base_cost ? styles.marginHealthy : styles.marginRisk}>{item.unit_price ? Math.round((item.unit_price * (1 - item.discount_percent / 100) - item.base_cost) / (item.unit_price * (1 - item.discount_percent / 100)) * 100) : 0}%</strong></label> : null}<label className={styles.fullField}>Incluye (una línea por beneficio)<textarea value={item.features.join("\n")} onChange={(event) => updateItem(item.id, { features: event.target.value.split("\n").map((value) => value.trim()).filter(Boolean) })} /></label></div></article>; })}</div></section>
