...
import type { DiscountRule, Quote, QuoteItem, QuotePlan, QuoteStatus, QuoteStrategy } from "@/lib/adminData";
import styles from "../os.module.css";

type QuoteView = Quote & { public_url: string };
type DataResponse = { quotes: QuoteView[]; plans: QuotePlan[]; discounts: DiscountRule[] };
...
export default function QuoteAdmin() {
  ...
  const load = useCallback(async () => { const data = await api<DataResponse>("/api/admin/quotes"); setQuotes(data.quotes); setPlans(data.plans); setDiscounts(data.discounts); }, []);
  useEffect(() => {
    let cancelled = false;
  ...
  return <main className={styles.osPage}>
    ...
    <div className={styles.segmented}><button className={tab === "quotes" ? styles.activeSegment : ""} onClick={() => setTab("quotes")}><Send size={17} /> Cotizaciones</button><button className={tab === "plans" ? styles.activeSegment : ""} onClick={() => setTab("plans")}><Layers3 size={17} /> Planes y ofertas</button><button className={tab === "discounts" ? styles.activeSegment : ""} onClick={() => setTab("discounts")}><BadgePercent size={17} /> Descuentos permitidos</button></div>
